        ...
        try:
            ...
            if "429" in msg or "rate_limit" in msg or "rate limit" in msg:
                if len(GROQ_KEYS) > 1:
                    ...
                    # Jika sudah mencoba semua kunci dan kembali ke indeks 0, istirahat 60 detik!
                    if current_key_idx == 0:
                        print(f"\n[!] Semua API Key limit. Istirahat 60 detik supaya server Groq reset...")
                        time.sleep(60)
                    else:
                        print(f"\n[!] Rate Limit! Ganti bensin ke API Key indeks ke-{current_key_idx} (Jeda 3s)...")
                        time.sleep(3)
                else:
                    wait = (2 ** attempt) * 5
                    print(f"\n[!] Rate Limit! Cuma ada 1 key, ngerem dulu {wait} detik...")
                    time.sleep(wait)
                continue
            raise
...
def gen_mc(field: str, status: str, n: int = 25) -> list:
    s = "Pembuat soal kompetisi Indonesia. Kembalikan HANYA JSON array valid tanpa teks lain."
...
def gen_click(field: str, status: str, n: int = 30) -> list:
    ...
    # TURUNKAN MAX TOKENS AGAR TIDAK DITENDANG GROQ
    return _j(_g(s, u, 4000))
